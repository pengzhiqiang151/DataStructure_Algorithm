...
class CartTree(object):
    ...
    def train(self, dt: np.array, depth: int = 0):
        ...
        if depth > self.depth or sum(dt[:, 0] != dt[0, 0]) == 0 or dt.shape[0] <= 10:
            node.predict = pd.DataFrame(node.data[:, 0]).mode().iloc[0, 0]  # 为叶节点赋值
            self.num_leaf += 1
            print('生成第 {} 个 叶节点'.format(self.num_leaf))
            return node
        # 寻找当前节点的分裂规则(特征、分裂值) [ind, value]   (dt[ind] ? value)
        node.rule = self.find_split(dt)
        # 递归创建左子树和右子树
        data_left = node.data[node.data[:, node.rule[0]] <= node.rule[1], :]
        data_right = node.data[node.data[:, node.rule[0]] > node.rule[1], :]
        if data_left.shape[0] == 0:  # 左节点样本数为空, 终止
            node.left = TreeNode()      # 生成左叶节点
            node.left.predict = pd.DataFrame(node.data[:, 0]).mode().iloc[0, 0]     # 为叶节点赋值
            self.num_leaf += 1
            print('生成第 {} 个 叶节点'.format(self.num_leaf))
            return
        if data_right.shape[0] == 0:  # 右节点样本数为空, 终止
            node.right = TreeNode()     # 生成右叶节点
            node.right.predict = pd.DataFrame(node.data[:, 0]).mode().iloc[0, 0]    # 为叶节点赋值
            self.num_leaf += 1
            print('生成第 {} 个 叶节点'.format(self.num_leaf))
            return
